...
function makeUserButton(){
  ...
  Object.keys(users).forEach((key) => {
    html += `<li data-uuid=${users[key].uuid} data-name=${users[key].name}> `;
    html += `<button class="btn btn-primary"> ${users[key].name} </button> </li>`
  })
  $("#users").empty();
  $("#users").html(html);
}
...
function checkPeerDisconnect(event, peerUuid, displayname) {
  ...
  if (state === "failed" || state === "closed" || state === "disconnected") {
    ...
    // makeUserButton();
    // document.getElementById('videos').removeChild(document.getElementById('localVideo' + peerUuid));
    updateLayout();
  }
}
